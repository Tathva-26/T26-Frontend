...
export function GlowLetters({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      if (img) {
        ...
        const fontPx = Math.min((w * textFit * probe) / mw, h * 0.85);
        textCtx.font = `${fontWeight} ${fontPx}px ${fontFamily}`;
        textCtx.textAlign = "center";
        textCtx.textBaseline = "middle";
        textCtx.fillStyle = "#ffffff";
        textCtx.fillText(text, w / 2, h * textY);
      }
  ...
  }, [text, imageSrc, textColor, textFit, textY, fontFamily, fontWeight, radius, intensity, follow, growSpeed, shrinkSpeed]);
